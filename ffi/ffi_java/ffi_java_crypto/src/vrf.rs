...
#[no_mangle]
/// Java interface for
/// 'com.webank.wedpr.crypto.NativeInterface->curve25519VrfProve'.
pub extern "system" fn Java_com_webank_wedpr_crypto_NativeInterface_curve25519VrfProve(
    _env: JNIEnv,
    _class: JClass,
    encoded_private_key_jstring: JString,
    encoded_input_jstring: JString,
) -> jobject {
    let result_jobject = get_result_jobject(&_env);

    let private_key_bytes =
        java_safe_jstring_to_bytes!(_env, result_jobject, encoded_private_key_jstring);
    let input_string = java_safe_jstring_to_string!(_env, result_jobject, encoded_input_jstring);
    let proof = match WedprCurve25519Vrf::prove(&private_key_bytes, &input_string) {
        Ok(v) => v,
        Err(_) => {
            return java_set_error_field_and_extract_jobject(
                ...
                &result_jobject,
                &format!(
                    "WedprCurve25519Vrf prove failed, private_key_bytes={}, input_bytes={}",
                    bytes_to_string(&private_key_bytes),
                    bytes_to_string(&input_string)
                ),
            )
        }
    };
...
#[no_mangle]
/// Java interface for
/// 'com.webank.wedpr.crypto.NativeInterface->curve25519VrfVerify'.
pub extern "system" fn Java_com_webank_wedpr_crypto_NativeInterface_curve25519VrfVerify(
    ...
    let public_key_bytes =
        java_safe_jstring_to_bytes!(_env, result_jobject, encoded_public_key_jstring);
    let input_string = java_safe_jstring_to_string!(_env, result_jobject, encoded_input_jstring);
    let proof_bytes = java_safe_jstring_to_bytes!(_env, result_jobject, encoded_proof_jstring);
    ...
    let result = proof.verify(&public_key_bytes, &input_string);
